...
- Rota `/insights/d1` = `PlaceholderPageComponent` hoje
...
- [ ] Step 1: Criar `core/api/models/insights-d1.model.ts` (DTOs RF-API-08, RF-API-11)
- [ ] Step 2: Criar `d1-aggregate.util.ts`, `d1-date.util.ts`, `d1-report-key.util.ts`
- [ ] Step 3: Exportar `getMockEnriquecidoRows(dt)` em `enriquecido-mock.data.ts`
- [ ] Step 4: Criar `core/api/data/insights-d1-mock.data.ts`
- [ ] Step 5: Criar `InsightsD1ApiService` — `getInsight(dt)`, `getDownload(dt)`
- [ ] Step 6: Implementar `InsightsD1FacadeService` (API + mock fallback)
- [ ] Step 7: Criar `D1DateSelectorComponent` (RF-M4-01)
- [ ] Step 8: Criar `D1InsightPanelComponent` (RF-M4-07)
- [ ] Step 9: Criar `D1RankingTableComponent` (RF-M4-02)
- [ ] Step 10: Criar `D1DownloadButtonComponent` (RF-M4-05)
- [ ] Step 11: Criar `D1MissingPartitionBannerComponent` (RF-M4-06)
- [ ] Step 12: Criar `InsightsD1PageComponent` (layout + estados + query `?dt=`)
- [ ] Step 13: Atualizar `app.routes.ts` — `/insights/d1` → `InsightsD1PageComponent`
- [ ] Step 14: Garantir home, enriquecido, origem inalterados (regressão specs)
- [ ] Step 15: Unit tests — aggregate PBT, facade, date/report utils, insight text
- [ ] Step 16: `scripts/w7-us07-validate.ps1`
- [ ] Step 17: Atualizar `portal-web/README.md` e `insights-d1/code-summary.md`
- [ ] Step 18: Atualizar `stories.md` E8-US07 → `done` após checklist manual
